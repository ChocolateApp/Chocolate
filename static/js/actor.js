...
function setPopup() {
    //get all elements with id cover
    actorMovie = document.getElementsByClassName("actorMovie")

    for (var i = 0; i < actorMovie.length; i++) {

        element = actorMovie[i]
        image = element.children[0]
        realTitle = image.getAttribute("title");
        ...
        element.addEventListener("click", function() {
            ...
            fetch("/getMovieData/" + realTitle).then(function(response) {
                ...
                for (var i = 0; i < movieCast.length; i++) {
                    ...
                    castImage.addEventListener("click", function() {
                        url = `/actor/${castName.innerHTML}`
