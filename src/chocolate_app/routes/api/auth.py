...
def image_to_base64(image_path: str, width: int | None = None, height: int | None = None) -> str:
    image_base64 = None
...
def login():
    ...
    return (generate_response(Codes.SUCCESS, False, {
        "user": user_object, "access_token": access_token, "refresh_token": refresh_token
    }))
...
def refresh():
    ...
    return (generate_response(Codes.SUCCESS, False, {
        "id": current_user.id, "name": current_user.name, "account_type": current_user.account_type, "access_token": access_token, "user": user_object
    }))
...
def get_accounts():
    ...
    for user in all_users:
        ...
        user_dict = {
            "name": user.name,
            "profile_picture": "data:image/jpeg;base64," + base64.b64encode(buffered.getvalue()).decode("utf-8"),
            "account_type": user.account_type,
