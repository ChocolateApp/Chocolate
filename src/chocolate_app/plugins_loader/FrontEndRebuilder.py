...
def rebuild_frontend():
    ...
    git_clone_command = "git clone https://github.com/ChocolateApp/ChocolateReact.git frontend_temp"

    if os.path.exists(frontend_path):
        shutil.rmtree(frontend_path, ignore_errors=True)
        pass

    subprocess.run(git_clone_command.split(), stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)

    frontends = loader.PLUGIN_FRONTENDS
    requirements = []
    for plugin_frontend in frontends:
        plugin_hash = generate_hash(plugin_frontend)
        requirements.extend(handle_frontend(plugin_frontend, frontend_path, plugin_hash))
...
def build_frontend(frontend_path: str, chocolate_path: str, requirements: list):
    ...
    subprocess.run(npm_install_command.split(), stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)

    for requirement in requirements:
        subprocess.run(f"npm install {requirement}".split(), stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    ...
    if not os.path.exists(f"{build_folder}/static"):
        raise ChocolateException("The build folder does not contain a static folder. You should check the error message above")
    ...
    for folder in os.listdir(f"{build_folder}/static"):
        if os.path.isdir(f"{build_folder}/static/{folder}"):
            print(f"Je copie {build_folder}/static/{folder} dans {chocolate_path}/static/{folder}")
            shutil.copytree(f"{build_folder}/static/{folder}", f"{chocolate_path}/static/{folder}", dirs_exist_ok=True)

    # le fichier index.html est copié dans chocolate_path/templates
    print(f"Je copie {build_folder}/index.html dans {chocolate_path}/templates/index.html")
...
def handle_frontend(plugin_frontend: str, frontend_path: str, plugin_hash: str) -> list:
    ...
    for frontend_dir in frontend_dirs:
        if os.path.exists(f"{plugin_frontend}/{frontend_dir}"):
            if frontend_dir == "New":
                rqrs = handle_new(f"{plugin_frontend}/{frontend_dir}", frontend_path, plugin_hash)
...
def handle_new(new_folder: str, frontend_path: str, plugin_hash: str):
    ...
    for files in os.listdir(new_folder):
        ...
        if os.path.isfile(file_path):
            ...
            new_file_code = re.sub(r'export default function (.*?)\(', f'export default function \\1_{plugin_hash}(', new_file_code)
            ...
            with open(app_file, 'r') as f:
                lines = f.readlines()
                last_import_index = next((i for i, line in enumerate(lines) if not line.startswith('import ')), len(lines))
                lines.insert(last_import_index, import_string + '\n')
                
                routes_end_index = next((i for i, line in enumerate(lines) if line.strip() == '</Routes>'), len(lines))
